...
import { createQuery, invalidate, subscribe } from '../src/query';
...
describe('createQuery', () => {
  ...
  it('caches based on stable key (object key order irrelevant)', () => {
    const fn = vi.fn().mockResolvedValue('data');
    const query = createQuery(uid(), (filter: { a?: number; b?: string }) => fn(filter));
  ...
  it('does not clear cache if a newer promise replaced the rejected one', async () => {
    const id = uid();
    let calls = 0;
    const fn = vi.fn().mockImplementation(() => {
      calls++;
    ...
    const p1 = query('a'); // call 1 — will reject
  ...
  it('handles non-Error rejection values', async () => {
    const query = createQuery(uid(), (_a: string) => Promise.reject('string-error'));
    await expect(query('a')).rejects.toBe('string-error');
  });
